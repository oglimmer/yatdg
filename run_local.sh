...
startDockerNetwork() {
	if [ -z "$DOCKER_NETWORKED_CHECKED" ]; then
		...
		if ! docker network ls | grep -s "yatdgnet"; then
			verbosePrint "Starting docker network yatdgnet on 10.120.203.0/24"
			docker network create -d bridge --subnet 10.120.203.0/24 --gateway 10.120.203.1 "yatdgnet"
		else
			verbosePrint "Docker network yatdgnet already running"
		fi
	fi
}
...
Url: http://localhost:8080/yatdg

Details for components:
yatdg {Source:\"mvn\", Default-Type:\"local\", Version-Info: \"Tested with 3-jre-11\"}
  -t yatdg:local #build local and respect -j
  -t yatdg:docker:[TAG] #docker based build, default tag: latest, uses image https://hub.docker.com/_/maven
...
if [ -z "$1" ]; then
	...
	declare START_YATDG=YES

	declare START_TOMCAT=YES

else
	ALL_COMPONENTS=(YATDG TOMCAT)
	for comp in "$@"; do
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="cacb1125b3e1a482128729efa6afd8bf"
...
verbosePrint "DEFAULT: TYPE_SOURCE_YATDG=local"
TYPE_SOURCE_YATDG=local
	...
	cat <<-EOF >Vagrantfile
		...
		Vagrant.configure("2") do |config|
		  ...
		  config.vm.provision "shell", inline: <<-SHELL
		    ...
		    echo "...then browse to http://localhost:8080/yatdg"
		  SHELL
		end
	EOF
	vagrant up
...
# MvnPlugin // yatdg
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
verbosePrint "MvnPlugin // yatdg"

if [ "$START_YATDG" = "YES" ]; then
	...
	for typeSourceElement in "${array[@]}"; do
		...
		if [ "$comp" == "yatdg" ]; then
			TYPE_SOURCE_YATDG=$type
			if [ "$TYPE_SOURCE_YATDG" == "local" ]; then
				TYPE_SOURCE_YATDG_PATH=$pathOrVersion
			else
				TYPE_SOURCE_YATDG_VERSION=$pathOrVersion
			fi
		fi

	done

	if [ "$TYPE_SOURCE_YATDG" == "docker" ]; then
		if [ -z "$TYPE_SOURCE_YATDG_VERSION" ]; then
			TYPE_SOURCE_YATDG_VERSION=latest
		fi

	fi

	verbosePrint "TYPE_SOURCE_YATDG = $TYPE_SOURCE_YATDG // TYPE_SOURCE_YATDG_PATH = $TYPE_SOURCE_YATDG_PATH // TYPE_SOURCE_YATDG_VERSION = $TYPE_SOURCE_YATDG_VERSION"
	...
	if [ "$TYPE_SOURCE_YATDG" == "docker" ]; then
		...
		f_build() {
			verbosePrint "pwd=$(pwd)\ndocker run --name=yatdg --rm -v $(pwd):/usr/src/build -v "$(pwd)/localrun/.m2":/root/.m2 -w /usr/src/build $dockerImage:$TYPE_SOURCE_YATDG_VERSION mvn $MVN_CLEAN $MVN_OPTS package"
...
if [ "$START_TOMCAT" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "docker" ]; then
		...
		if [ ! -f ".tomcatPid" ]; then
			...
			verbosePrint "docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 -m 70M --net=yatdgnet --name=tomcat $ADD_HOST_INTERNAL   -v "$(pwd)/localrun/webapps":/usr/local/tomcat/webapps oglimmer/adoptopenjdk-tomcat:$TYPE_SOURCE_TOMCAT_VERSION"
			dockerContainerIDtomcat=$(docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 \
				-m 70M \
				--net=yatdgnet --name=tomcat $ADD_HOST_INTERNAL \
				-v \
...
if [ "$TAIL" == "YES" ]; then
	echo "http://localhost:8080/yatdg"
